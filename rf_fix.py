...
Original file is located at
    https://colab.research.google.com/drive/1sWQOvpUh_Co0lLb6bApRvPFMVaMr1Bya

**<h1 style="font-size:36px;"> Klasifikasi Data Stunting Kabupaten Pemalang Dengan Random Forest- Rangga Aditya Nugraha</h1>**
"""
...
print("\n🔎 Korelasi antara fitur dan target (stunting):")
...
# ---------------------------------------------------------------------------------------------------------------------------
# Jika tidak ada fitur yang memenuhi threshold → pakai semua fitur numerik saja
# ---------------------------------------------------------------------------------------------------------------------------
if len(selected_features) == 0:
    print(f"\n⚠️ Tidak ada fitur dengan korelasi ≥ {threshold}. Semua fitur numerik akan digunakan untuk PCA.")
    selected_features = x.select_dtypes(include=[np.number]).columns.tolist()
else:
    print(f"\n✅ Fitur yang dipilih berdasarkan korelasi ≥ {threshold}:")
...
if x_selected.shape[1] == 0:
    raise ValueError("❌ Tidak ada kolom numerik yang tersedia untuk PCA. Periksa kembali tipe data fitur.")
...
# ---------------------------------------------------------------------------------------------------------------------------
print("📌 Cek missing values di x_train:")
...
if imbalance_ratio > 1.5:
    ...
    print("\n✅ Dataset sudah seimbang, SMOTE tidak diterapkan.")
...
# Sebelum SMOTE
# ---------------------------------------------------------------------------------------------------------------------------
axes[0].bar(["Tidak Stunting", "Stunting"], class_distribution_before.values(), color='#D1B2FF')
...
axes[1].bar(["Tidak Stunting", "Stunting"], class_distribution_after.values(), color='#7B3FF0')
...
# Preview data
# ---------------------------------------------------------------------------------------------------------------------------
print("\n📄 Preview Data Training Hasil SMOTE + Scaling:")
print(df_train_scaled.head())

# ---------------------------------------------------------------------------------------------------------------------------
# Statistik setelah scaling
# ---------------------------------------------------------------------------------------------------------------------------
print("\n📊 Statistik numerik setelah scaling:")
...
print("\n📊 Data Training dan Testing TANPA SMOTE telah di-scale!")
...
# -------------------------
# 1️⃣ Standarisasi data
# -------------------------
scaler_ns = StandardScaler()
X_train_ns = pd.DataFrame(x_train_imputed_scaled, columns=x_train_imputed.columns)
X_test_ns  = pd.DataFrame(x_test_imputed_scaled, columns=x_test_imputed.columns)
y_train_ns = y_train.copy()
y_test_ns  = y_test.copy()
...
rf_ns = RandomForestClassifier(
    ...
    min_samples_leaf=3,
    max_features='sqrt',
    class_weight='balanced',   # penting karena data tidak seimbang
    random_state=42,
    n_jobs=-1
)

# -------------------------
# 3️⃣ Training & Evaluasi
# -------------------------
rf_ns.fit(X_train_ns, y_train_ns)
y_train_pred_ns = rf_ns.predict(X_train_ns)
y_test_pred_ns = rf_ns.predict(X_test_ns)

print("\n📊 HASIL RANDOM FOREST TANPA SMOTE:")
print(f"Train Accuracy: {accuracy_score(y_train_ns, y_train_pred_ns):.4f}")
print(f"Test Accuracy : {accuracy_score(y_test_ns, y_test_pred_ns):.4f}")
print("\nClassification Report (Test):")
print(classification_report(y_test_ns, y_test_pred_ns))

# Confusion Matrix
cm_ns = confusion_matrix(y_test_ns, y_test_pred_ns)
plt.figure(figsize=(5,4))
sns.heatmap(cm_ns, annot=True, fmt='d', cmap='Blues',
            xticklabels=np.unique(y_test_ns), yticklabels=np.unique(y_test_ns))
plt.title("Confusion Matrix - Random Forest TANPA SMOTE")
plt.xlabel("Predicted"); plt.ylabel("True")
plt.show()

# ==========================================================
# 🟦 RANDOM FOREST DENGAN SMOTE + STANDARISASI
# ==========================================================
from sklearn.ensemble import RandomForestClassifier
from sklearn.metrics import accuracy_score, classification_report, confusion_matrix
import seaborn as sns
...
# Gunakan hasil SMOTE dan scaling yang sudah ada dari blok preprocessing kamu
X_train_s = x_train_res_scaled.copy()
X_test_s  = x_test_res_scaled.copy()
y_train_s = y_train_res.copy()
...
rf_s = RandomForestClassifier(
    ...
    min_samples_leaf=3,
    max_features='sqrt',
    class_weight=None,     # tidak perlu balanced, karena SMOTE sudah menyeimbangkan data
    random_state=42,
    n_jobs=-1
)
...
y_train_pred_s = rf_s.predict(X_train_s)
y_test_pred_s = rf_s.predict(X_test_s)

print("\n📊 HASIL RANDOM FOREST DENGAN SMOTE:")
print(f"Train Accuracy: {accuracy_score(y_train_s, y_train_pred_s):.4f}")
print(f"Test Accuracy : {accuracy_score(y_test_s, y_test_pred_s):.4f}")
print("\nClassification Report (Test):")
print(classification_report(y_test_s, y_test_pred_s))

# Confusion Matrix
cm_s = confusion_matrix(y_test_s, y_test_pred_s)
plt.figure(figsize=(5,4))
sns.heatmap(cm_s, annot=True, fmt='d', cmap='Purples',
            xticklabels=np.unique(y_test_s), yticklabels=np.unique(y_test_s))
plt.title("Confusion Matrix - Random Forest DENGAN SMOTE")
plt.xlabel("Predicted"); plt.ylabel("True")
plt.show()

# ==========================================================
# 🟧PERBANDINGAN KINERJA MODEL (TANPA vs DENGAN SMOTE)
# ==========================================================
...
metrics_no_smote = {
    "Accuracy": accuracy_score(y_test_ns, y_test_pred_ns),
    "Precision": precision_score(y_test_ns, y_test_pred_ns, zero_division=0),
    "Recall": recall_score(y_test_ns, y_test_pred_ns, zero_division=0),
    "F1-Score": f1_score(y_test_ns, y_test_pred_ns, zero_division=0)
}

# Hitung metrik untuk model DENGAN SMOTE
metrics_smote = {
    "Accuracy": accuracy_score(y_test_s, y_test_pred_s),
    "Precision": precision_score(y_test_s, y_test_pred_s, zero_division=0),
    "Recall": recall_score(y_test_s, y_test_pred_s, zero_division=0),
    "F1-Score": f1_score(y_test_s, y_test_pred_s, zero_division=0)
}
...
print("📊 PERBANDINGAN KINERJA MODEL RANDOM FOREST:")
display(df_compare)
...
df_compare.plot(kind="bar", color=["#A3D8F4", "#4B88A2", "#F9ED69", "#F08A5D"], figsize=(8, 5))
plt.title("Comparation Performance Rf Imbalanced vs With SMOTE", fontsize=13)
...
# ==========================================================
# HYPERPARAMETER TUNING RANDOM FOREST (CROSS-VALIDATION) (NON-SMOTE)
# ==========================================================
from sklearn.ensemble import RandomForestClassifier
...
print("\n🔧 Hyperparameter Tuning Random Forest TANPA SMOTE...")
...
rf_tuned_ns = RandomForestClassifier(random_state=42, n_jobs=-1, class_weight='balanced')

grid_search_rf_ns = GridSearchCV(
    estimator=rf_tuned_ns,
    param_grid=param_grid_rf_ns,
    cv=5,
    scoring='accuracy',
    n_jobs=-1,
    verbose=1
...
grid_search_rf_ns.fit(X_train_ns, y_train_ns)

print("\n✅ Hasil Hyperparameter Tuning Random Forest (Tanpa SMOTE):")
print(f"Best Parameters: {grid_search_rf_ns.best_params_}")
print(f"Best Cross-Validation Accuracy: {grid_search_rf_ns.best_score_:.4f}")

# Evaluasi model terbaik
best_rf_ns = grid_search_rf_ns.best_estimator_
y_pred_best_rf_ns = best_rf_ns.predict(X_test_ns)

print("\n📊 Laporan Klasifikasi (Model Terbaik Tanpa SMOTE):")
print(classification_report(y_test_ns, y_pred_best_rf_ns))

cm_rf_ns = confusion_matrix(y_test_ns, y_pred_best_rf_ns)
plt.figure(figsize=(5, 4))
sns.heatmap(cm_rf_ns, annot=True, fmt='d', cmap='Reds', cbar=False)
plt.title("Confusion Matrix — Random Forest (Best Model TANPA SMOTE)")
plt.xlabel("Predicted Label")
plt.ylabel("True Label")
plt.show()

# Bandingkan akurasi
acc_default_ns = accuracy_score(y_test_ns, y_test_pred_ns)
acc_tuned_ns   = accuracy_score(y_test_ns, y_pred_best_rf_ns)
print(f"\n📈 Akurasi Model Default RF (Tanpa SMOTE): {acc_default_ns:.4f}")
print(f"🏆 Akurasi Model RF Setelah Tuning (Tanpa SMOTE): {acc_tuned_ns:.4f}")

# ==========================================================
# HYPERPARAMETER TUNING RANDOM FOREST (CROSS-VALIDATION)
# ==========================================================
...
# Gunakan data SMOTE + scaling dari blok RF sebelumnya
...
print(classification_report(y_test_s, y_pred_best_rf))

# ==========================================================
# Confusion Matrix
# ==========================================================
cm_rf = confusion_matrix(y_test_s, y_pred_best_rf)
...
# ==========================================================
# Perbandingan Akurasi dengan RF default
# ==========================================================
acc_default = accuracy_score(y_test_s, y_test_pred_s)
acc_tuned   = accuracy_score(y_test_s, y_pred_best_rf)

print(f"\n📈 Akurasi Model Default RF: {acc_default:.4f}")
print(f"🏆 Akurasi Model RF Setelah Tuning: {acc_tuned:.4f}")

# ==========================================================
# Perbandingan Akurasi Training vs Testing (criterion: gini vs entropy)
# ==========================================================
...
# Range jumlah pohon (n_estimators)
n_values = range(10, 310, 10)
criterion_options = ['gini', 'entropy']  # dua varian kriteria pemisahan
...
for i, crit in enumerate(criterion_options):
    ...
    for n in n_values:
        ...
        # Hitung akurasi
        train_acc.append(model.score(X_train_s, y_train_s))
        test_acc.append(model.score(X_test_s, y_test_s))

    # Subplot untuk setiap criterion
    plt.subplot(1, 2, i+1)
    plt.plot(n_values, train_acc, 'o-', label='Train Accuracy')
    plt.plot(n_values, test_acc, 's--', label='Test Accuracy')
    plt.xlabel('n_estimators')
    plt.ylabel('Accuracy')
    plt.title(f"criterion = '{crit}'")
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.ylim(0.6, 1.05)

    # Titik terbaik (jumlah pohon dengan akurasi test tertinggi)
    best_n = n_values[np.argmax(test_acc)]
    best_test_acc = np.max(test_acc)
    plt.axvline(best_n, color='gray', linestyle=':')
    plt.text(best_n, best_test_acc - 0.02,
             f"best n={best_n}\nTest={best_test_acc:.3f}",
             color='black', fontsize=9)

    plt.legend()

plt.suptitle("Perbandingan Akurasi Training vs Testing untuk variasi jumlah pohon (Random Forest)")
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.show()

# ==========================================================
# Grafik Perbandingan Train vs Test (criterion: gini vs entropy)
# ==========================================================
from sklearn.ensemble import RandomForestClassifier
import numpy as np
import matplotlib.pyplot as plt
...
for i, crit in enumerate(criterion_options):
    ...
    for n in n_values:
        ...
        train_acc.append(model.score(X_train_ns, y_train_ns))
        test_acc.append(model.score(X_test_ns, y_test_ns))
    ...
    plt.xlabel('n_estimators')
    plt.ylabel('Accuracy')
    plt.title(f"criterion = '{crit}' (Tanpa SMOTE)")
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.ylim(0.6, 1.05)
...
plt.suptitle("Perbandingan Akurasi Train vs Test (Tanpa SMOTE)")
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.show()

"""# **RANDOM FOREST (FEATURE IMPORTANCE & ROC)**"""

# ==========================================================
# 📊 VISUALISASI RANDOM FOREST (FEATURE IMPORTANCE & ROC)
# Letakkan kode ini di cell paling bawah Google Colab
# ==========================================================
...
# ==========================================================
# 1️⃣ FEATURE IMPORTANCE (Dari Model RF SMOTE Terbaik)
# ==========================================================

# Ambil importance dari model terbaik (best_rf)
importances = best_rf.feature_importances_

# Ambil nama fitur dari DataFrame X_train_s
...
plt.title("Feature Importance (Random Forest Optimized)", fontsize=14)
...
print("✅ Gambar 1 Berhasil: rf_feature_importance.png")
plt.show()

# ==========================================================
# 2️⃣ ROC CURVE COMPARISON (RF SMOTE vs RF NON-SMOTE)
# ==========================================================

# Hitung Probabilitas
# Model SMOTE (best_rf) menggunakan data test SMOTE (X_test_s)
y_prob_rf_smote = best_rf.predict_proba(X_test_s)[:, 1]

# Model Non-SMOTE (best_rf_ns) menggunakan data test Non-SMOTE (X_test_ns)
y_prob_rf_ns = best_rf_ns.predict_proba(X_test_ns)[:, 1]

# Hitung Metrik ROC untuk SMOTE
fpr_rf, tpr_rf, _ = roc_curve(y_test_s, y_prob_rf_smote)
roc_auc_rf = auc(fpr_rf, tpr_rf)

# Hitung Metrik ROC untuk Non-SMOTE
fpr_rf_ns, tpr_rf_ns, _ = roc_curve(y_test_ns, y_prob_rf_ns)
roc_auc_rf_ns = auc(fpr_rf_ns, tpr_rf_ns)

# Plotting
plt.figure(figsize=(8, 6))

# Garis RF SMOTE (Harusnya lebih bagus)
plt.plot(fpr_rf, tpr_rf, color='blue', lw=2,
         label=f'RF w/ SMOTE (AUC = {roc_auc_rf:.2f})')

# Garis RF Non-SMOTE (Sebagai pembanding)
plt.plot(fpr_rf_ns, tpr_rf_ns, color='orange', lw=2, linestyle=':',
         label=f'RF Imbalanced (AUC = {roc_auc_rf_ns:.2f})')
...
# Simpan Gambar
plt.savefig('rf_roc_curve_comparison.png', dpi=300)
print("✅ Gambar 2 Berhasil: rf_roc_curve_comparison.png")
plt.show()